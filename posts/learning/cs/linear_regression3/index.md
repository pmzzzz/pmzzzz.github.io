...
通过使用深度学习框架来简洁地实现 [线性回归(2)--从零实现](../linear_regression2/)中 的(线性回归模型)
<!--more-->
{{<admonition tip "注意">}}
本文主要记录代码，优化细节，添加注释。
{{</admonition>}}
## 细节highlight
1. `torch.utils.data`很方便地处理数据集[读取数据](#读取数据)
1. `nn.Sequential`用序列的方式定义网络，方便直观，不需要自己写forward[定义模型](#定义模型)
1. 训练步骤: output-->loss-->zero_grad()-->backward()-->step()
前向传播-->计算loss-->梯度清零-->反向传播-->更新参数  [训练模型](#训练模型)
